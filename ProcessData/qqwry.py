...
if __name__ == '__main__':
	import sys
	path='C:/Users/Cathy/Desktop/通识课/1001667002-中国茶道'
	filename='wda_mooc_1001667002.txt'
	dirname=path.split('/')[-1]
	DataPreprocessing.setpath(dirname)
	DataListA=[]
	DataListA=DataPreprocessing.data(path+'/'+filename,"uid")
	DataListB=[]
	DataListB=DataPreprocessing.data(path+'/'+filename,"ip")
	DataListC=[]
	#DataListC=DataPreprocessing.data(path+'/wda_mooc/wda_mooc_268001.txt',"gmt_create")
	anslist=[]
	anslist=DataPreprocessing.user_ip_dictmap(DataListA,DataListB)
	DataListB=[]
	DataListB=anslist
	LogTime=[]
	LogTime=DataPreprocessing.ChangeTime1(DataListC)
	#qqwry_place=open(path+'/qqwry_place1.txt', 'w+')
	school_user=open(path+'/school_user.txt', 'w+')
	city_user=open(path+'/city_user.txt', 'w+')
	fn = 'qqwry.dat'
	q = QQwry()
	q.load_file(fn)
	provinceDic={}
	...
	college_user=[]
	schoolCode=[]
	for i in range(0, len(DataListB)):
		flag2=q.lookup(DataListB[i])
		if flag2 is None:
			continue
		s1 = str(flag2[0])
		flag = 0
		if '省' in s1:
			s11 = s1[0:s1.rfind('省')+1]
			flag = s1.rfind('省')+1
			if s11 in provinceDic.keys():
				provinceDic[s11]+=1
				s11 = str(provinceCode.index(s11))
			else:
				provinceDic[s11]=1 
				provinceCode.append(s11)
				s11 = str(len(provinceCode)-1)
			if  '市' in s1:
				lastflag = s1.rfind('市')+1
				s12 = s1[flag:lastflag]
				s13 = s12[0:-1]
				fflag = s13.rfind('市')
				if fflag != -1:
					s12 = s12[fflag+1:lastflag]
				if s12 in cityDic.keys():
					cityDic[s12]+=1
					s12 = str(cityCode.index(s12))
				else:
					cityDic[s12]=1 
					cityCode.append(s12)
					s12 = str(len(cityCode)-1)
			else:
				s12 = '*'
		else:
			s11 = '*'
		...
		s2 = str(flag2[1])
		if '学院' in s2:
			s2 = s2[0:s2.rfind('学院')+2]
			if s2 in schoolDic.keys():
				schoolDic[s2]+=1
				s2 = str(schoolCode.index(s2))
			else:
				schoolDic[s2]=1 
				schoolCode.append(s2)
				s2 = str(len(schoolCode)-1)
		elif  '大学' in s2:
			s2 = s2[0:s2.rfind('大学')+2]
			if s2 in schoolDic.keys():
				schoolDic[s2]+=1
				s2 = str(schoolCode.index(s2))
			else:
			...
			s2 = '*'
		#qqwry_place.write(DataListA[i]+','+LogTime[i]+','+DataListB[i]+','+s11+','+s12+','+s2+'\n')
	flagdic = {}
	#d = json.load(open('college.txt','r'))
	for key in schoolDic:
		flagdic['name'] = key
		flagdic['number'] = schoolDic[key]
		college_user.append(flagdic)
		flagdic = {}
	school_user.write(str(college_user))
	#school_user1=open(path+'/school_user1.txt', 'w+')
	#json.dump(college_user, school_user1)
	school_user.close()
	#school_user1.close()
	area_user = []
	for key in cityDic:
		flagdic['name'] = key
		flagdic['number'] = cityDic[key]
		area_user.append(flagdic)
		flagdic = {}
	city_user.write(str(area_user))
	#city_user1=open(path+'/city_user1.txt', 'w+')
	#json.dump(area_user, city_user1)
	city_user.close()
	#city_user1.close()
	'''
	file=open('provinceCode.txt','w') 
	file.write(str(provinceCode)); 
	file.close() 
	file=open('cityCode.txt','w') 
	file.write(str(cityCode)); 
	file.close() 
	file=open('schoolCode.txt','w') 
	file.write(str(schoolCode)); 
	file.close()
	'''
	print('end')